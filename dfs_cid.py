...
class DFS(object):
    ...
    def exists(self, cid, parent, name):
        return 0 < self.lookup(cid, parent, name)
    
    # see where else we need cid
    def mknod(self, cid, parent, name, mode, mtime):
        # check if the file exists
        if self.exists(cid, parent, name):
            assertion(False)
...
# Will this run when the fs is mounted?
if __name__ == '__main__':
    dfs = create_dfs()

    # TODO: print some useful code like this (but using clients) :)
